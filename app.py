...
def fetch(url: str, cookie_str: str = '') -> str:
    ...
    resp = requests.get(url, impersonate="chrome146", timeout=20, headers=headers)
    ...
    with sync_playwright() as p:
        ...
        if not logged_in:
            if not sys.stdin.isatty():
                print("  → Not logged in and running non-interactively — skipping GP tab.", file=sys.stderr)
                ctx.close()
                return False
